...
namespace Demo {
    ...
    void Canvas::Render()
    {
        ...
        {
            ImVec2 p1( (origin.x + lines[n].x * scale), (origin.y + lines[n].y * scale));
            ImVec2 p2( (origin.x + lines[n].z * scale), (origin.y + lines[n].w * scale));
        ...
        for (int n = 0; n < points.Size; n += 1)
        {
            ImVec2 cen(origin.x + points[n].x * scale, origin.y + points[n].y * scale);
            draw_list->AddCircleFilled(cen, scale/4.0f, IM_COL32(255, 255, 0, 255));
        }
